...
problem that avoid the solver to visit symmetrical search space.
...
\input{algo/cdcl_cosy}
 ...
 \texttt{generateEsbp} computes the $\eta(\alpha, g)$ of~\Cref{def:eta},
the effective symmetry-breaking predicate of~\Cref{prop:eta}. This will
...
%
% \textcolor{red}{rajouter un paragraphe sur une exp̩rience pour lever le doute
...
The backtrack and the pruning capacity depend heavily on the chosen constraint. In our library, the first reducer permutation generates the constraint. 
...
takes the lead over esbp. This can be, specially, useful on SAT problems because esbp can eliminate non lex-leader SAT assignment.
To emphasize this behavior, the conflict of the esbp can be ignored in the sense that it is just added into clause database
and so participate to the next unit propagation. This gives to the solver the ability to find a solution
symmetrical branch but avoid to get multiple times on non-minimal part of search. 



\subsection{Change the Order Dynamically}
As seen before, the ordering relationship between variables influence the minimal value of each orbit and the generated constraints. 
...
can decide quickly if the current assignment is a reducer or not.
...
a scatter plot that compares the same variables order with $\true < \false$ and $\false < \true$,
...
\begin{figure}[!htbp]
	...
	\vspace*{0.1cm}
	\caption{Comparison of the order with different sign on 500 symmetric instances.}
	\label{table:compare_sign_order}
\end{figure}


\texttt{MiniSymTF} is generally better and is the default choice in the library.
If it is running on specific application, reverse order can be chosen if it performs better results.
