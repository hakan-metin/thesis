...
\hakan{NIVEAU MACROSCOPIQUE FORMULE MICROSCOPIQUE CLAUSE}
...
Nowadays, Boolean satisfiability (SAT) is an active research area finding its application in different domains
such as logic optimization, test pattern generation, formal verification, functional simulation, etc.
Hence, the development of approaches that could treat increasingly challenging SAT problem has become a focus.
Some problems exhibit symmetries and create a combinatorial explosion that hinders solver performance.
This one explores fruitlessly the symmetric parts of the search space.
...
The second called \textit{dynamic symmetry breaking} adds a symmetry component to the solver core
...
For this purpose, we propose two major contributions that aims to improve the state-of-the-art approaches and augment the number of solved symmetric instances. The first one mimics the behavior of \textit{static symmetry breaking} approach but operates
dynamically. A symmetry controller observes the behavior of the solver and generates constraints opportunistically.
