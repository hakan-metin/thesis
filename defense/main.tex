...
\frametitle{Motivation}
	...
	%	\node[opacity=0, onslide={<2>,opacity=1}](gr) at ($(coc) + (0pt, 130pt)$) {\includegraphics[width=.2\textwidth]{images/graphcoloring}};
		...
		\node[](p) at ($(coc) + (0pt, -60pt)$) {\includegraphics[width=.25\textwidth]{images/plan2}};
	   	...
	   	\node[rounded, fill=black!20, minimum height=2cm, minimum width=2cm] (sat) at ($(coc) + (120pt, 40pt)$) {SAT};
...
\begin{center}
	\visible<2->{YES! SAT $\alpha = (A,1), (B,2), (C, 3)$\\ }
...
\node[rounded,minimum width=3.5cm] (solver) at ($(input) + (0pt, -30pt)$) {SAT solver};
...
\begin{frame}
\frametitle{Our contribution CDCL[Sym]~\cite{metin2018cdclsym}}
