...
\begin{frame}
	...
	\tikzset{%
		...
		focus/.style = {draw=blue,line width=0.5mm},
		questioncurr/.style = {question,line width=0.7mm, draw=purple!80},
		line/.style = {->, thick },
		linecurr/.style = {line, line width=0.7mm, draw=purple!80},
	}
	
	\node (isfin) [question,onslide={<10>,focus}] {All vars\\ assigned?};
	\node (dec)     [process,onslide={<2>,focus}] at ($(isfin) + (0pt, -50pt)$)    {Choose decision var};
	\node (sdec)     [right = of dec]          {Start};
	\node (idec) [left = of isfin]  {};
	\node (prop)    [process,onslide={<3,8>,focus}] at ($(dec) + (0pt, -30pt)$)          {Unit propagation};
	
	
	\node (conf)    [question,onslide={<4,9>,focus}] at ($(prop) + (0pt, -40pt)$){ Is conflict?};
	\node (confanalyse) [process,onslide={<5>,focus}] at ($(conf) + (0pt, -60pt)$) {Conflict analysis};
	\node (learn) [process,onslide={<7>,focus}] at ($(prop) + (90pt, 0)$) {Learn conflict clause\\ and backjump};
	\node (isend) [question,onslide={<6>,focus}] at ($(confanalyse) + (90pt, 0)$) {is level\\zero?};
	\node (end) [unsat,double=black!10,double distance=0.5mm] at ($(isend) + (90pt, 0)$) {$unsatiasfiable$};
	\node (ends) [sat,double=black!10,double distance=0.5mm,,onslide={<11>,focus}] at ($(isfin) + (+90pt, 0)$){$satisfiable$};
...
\visible<3>{
\begin{tikzpicture}[overlay]
\node[](p) at (15pt, 75pt) {\includegraphics[width=.2\textwidth]{images/plan2}};
\node[](p) at (300pt, 75pt) {\includegraphics[width=.2\textwidth]{images/plan2}};
\end{tikzpicture}
}
\end{frame}
...
\begin{frame}
	...
	Symmetry controller:
	\begin{itemize}
		\item generates SBP on the fly 
		\item only when needed
		\item intrusive on solver
	\end{itemize}
...
\begin{frame}
\frametitle{Is assignment minimal? Symmetry status}
...
Observation:\\
\centering \textcolor{orange}{Cannot handle some instances solved by static approach}
}
...
\begin{frame}
	\frametitle{Local symmetry}
		...
		\begin{column}[T]{0.6\textwidth}
			...
			\node[,opacity=0, onslide={<4->,opacity=1}] (l) at (-1, 0.1){\textcolor{orange}{$\omega_{14}$}};
			
			\node[,opacity=0, onslide={<2->,opacity=1}] (lpp) at (-2.75, 1.60){\textcolor{purple}{$\omega_{13}$}};
