...
\begin{tikzpicture}[overlay]
	...
	\node[,opacity=0,onslide={<3->,opacity=1}] (ft) at ($(f) + (+90pt, 0pt)$) {CNF};
...
\node[](p) at ($(input) + (100pt, 0pt)$) {\includegraphics[width=.25\textwidth]{images/plan2}};
...
\node[rounded,minimum width=3.5cm, minimum height=1cm,opacity=0,line width=0.7mm,onslide={<2>,opacity=1}] (so) at ($(input) + (0pt, -40pt)$) {SAT solver};
...
\begin{frame}
	...
	Define an ordering relation to compare assignment
	\begin{itemize}
...
\vfill
	...
	Adds Symmetry breaking predicates (SBP)
...
\begin{frame}
\centering
\textcolor{UPMCEngagementBlueB}{\Large Our contribution CDCL[sym] }\\
\vspace{2em}
...
\begin{itemize}
	...
	\item filter: \bliss{} finds symmetries in $1000$s
...
\begin{frame}
	\frametitle{Learning symmetrical clauses}
	...
	\begin{columns}[t]
		...
		\begin{column}[T]{0.6\textwidth}
			...
			\tikzstyle{class1}=[ellipse, draw, minimum width=6cm, minimum height=5cm]
			...
			\node[pointl,opacity=0, onslide={<3>,opacity=1}] (p7) at (0.5, -1.5){};
			
			\node[pointl,opacity=0, onslide={<3>,opacity=1}] (pl) at (0.5, 1.5){};
			
			\node[point] (p7a) at (0.9, -0.87){};
			%	\node[](a7) at (0.5, -1.25) {$\alpha_7$};
			
			\node[point] (p8) at (-1, 2){};
			
			\node[point] (po) at (-3 , -0.2){};
			%	\node[](a8) at (-1, 2.25) {$\alpha_8$};
			
			\node[pointl,opacity=0, onslide={<2->,opacity=1}] (l) at (-1, 0.1){};
			
			
			
			\node[ellipse, draw, minimum width=6cm, minimum height=5cm] (c4) at (-1, 0) {};
