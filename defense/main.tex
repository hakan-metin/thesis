...
\frametitle{Motivation}
	...
	%	\node[opacity=0, onslide={<2>,opacity=1}](gr) at ($(coc) + (0pt, 130pt)$) {\includegraphics[width=.2\textwidth]{images/graphcoloring}};
		...
		\node[align=center] at ($(coc) + (-100pt, 0pt)$) {Hardware verification};
...
\draw[->, thick] (hlp) to node[fill=white]{encoding} (input);
...
\node[opacity=0,onslide={<5,9>,opacity=1}] (propm)    [process]  at (280pt, 200pt)          {Unit Propagation};%
...
\begin{frame}
		\frametitle{Comparing assignments (2/2)}
		...
		\vfill
		\normalsize
		\centering \visible<3->{ $g.\alpha \prec \alpha$} \visible<4->{$\Rightarrow$ Generate SBP $\omega = \{ \neg x_1, x_2\}$}
\end{frame}
%% ----------------------------------------------------------------------------
\begin{frame}
...
\begin{tikzpicture}
	...
	\node[box,minimum height=1.2cm,onslide={<3>, starburst,draw=red,line width=0.5mm}] (sbp) at (0, -2) { SBPs};
...
\begin{itemize}
	\item Shatter \cite{aloul06} \hspace{2em} \textcolor{UPMCEngagementBlueB}{$\bullet$}  BreakID \cite{devriendt2016improved}
\end{itemize}
...
\node (isfin) [question,onslide={<2->,fill=black!30}] {All vars\\ assigned?};
\node (dec)     [process,onslide={<2->,fill=black!30}] at ($(isfin) + (0pt, -50pt)$)    {Choose decision var};
\node[] (sdec)     [right = of dec]          {Start};
\node (idec) [left = of isfin]  {};
\node (prop)    [process,onslide={<2->,fill=black!30}] at ($(dec) + (0pt, -30pt)$)          {Unit propagation};
...
\node (ends) [sat,double=black!10,double distance=0.5mm,onslide={<2->,fill=black!30}] at ($(isfin) + (+90pt, 0)$){$satisfiable$};
