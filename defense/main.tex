...
\begin{frame}
	...
	\node (dec)     [process,onslide={<1->,focus}] at ($(isfin) + (0pt, -50pt)$)    {Choose decision var};
	\node (sdec)    at ($(dec) + (180pt, 0pt)$)          {Start};
	\node (idec) [left = of isfin]  {};
	\node (prop)    [process,onslide={<2->,focus}] at ($(dec) + (0pt, -30pt)$)      {Unit propagation};
...
\node[](p) at (300pt, 75pt) {\includegraphics[width=.2\textwidth]{images/planinv}};
...
\begin{frame}
	\frametitle{Discussion of improvements}
	...
	\visible<2->{
	 Composing permutations
		\begin{itemize}
			...
			\item Allow faster detection 
		\end{itemize}
	}
\vfill
	\visible<3->{
	Adapt solver heuristic dynamically
	\begin{itemize}
		\item Restart
...
\begin{frame}
	...
	\visible<2->{
		Learnt clauses are logical consequence of the formula\\
	}
	\visible<3>{
		\vfill
		$g \in G$ are symmetries of the formula \\$\rightarrow$ symmetrical learnt clause are logical consequence too
	}
...
\begin{frame}
	...
	\begin{center}
		\textbf{Guarantee that symmetrical clauses are logical consequence of the formula}
	\end{center}
...
\begin{columns}[t]
	\begin{column}[t]{.3\textwidth}
		...
		\visible<3->{\textcolor{purple}{$\omega_{13}$} $\leftarrow \{g_1 \}$}\\
		\visible<5->{\textcolor{orange}{$\omega_{14
				}$} $\leftarrow \{g_1, g_3\}$}
	\end{column}
\begin{column}[t]{.6\textwidth}
	\begin{itemize}
...
\begin{frame}
	...
	SP and ESBP have its own update $\rightarrow$ costly
	...
	\begin{itemize}
		\item SEL have few requirements than SP
		\item We hope that improve more the performance
	\end{itemize}
}
\end{frame}
