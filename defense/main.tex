...
\end{itemize}	
	...
	\begin{itemize}
		...
		\item[] 	$(x_1 \enskip x_2)(x_4 \enskip x_5)(x_7 \enskip x_8) $\\
	\end{itemize}
...
\begin{itemize}
	\item \texttt{reducer}: $\sigma(\alpha) \prec \alpha$
	\item \texttt{inactive} : $\alpha \prec \sigma(\alpha)$
...
\vfill
	...
	\begin{enumerate}
		\item $\alpha(\sigma(x)) \leq \alpha(x),$ \small then $\sigma$ is \texttt{reducer}
		$\Rightarrow$ Effective SBP (ESBP)
		\item $ \alpha(x) \leq \alpha(\sigma(x)),$ then $\sigma$ is \texttt{inactive}
		$\Rightarrow \sigma$ cannot reduce $\alpha$
		\item $\alpha(\sigma(x))$ or $\alpha(x)$ is unassigned then $\sigma$ is
		\texttt{active}
	\end{enumerate}
\end{block}  
\vspace{2em}
