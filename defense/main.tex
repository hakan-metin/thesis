...
\begin{frame}
	...
	\begin{tabular}{CC}
		...
		\visible<3-> {
			$\|$ & $\|$  \\
			graph automorphism & 
			\small{(\texttt{bliss}\footnote{http://www.tcs.hut.fi/Software/bliss/},
				\texttt{saucy} \footnote{http://vlsicad.eecs.umich.edu/BK/SAUCY/},$\cdots$)
			} 
			\\
		}
...
\visible<2>{\centering Works under \textbf{some conditions}}
...
\node[opacity=0,onslide={<2>,opacity=1},draw, rounded corners=4pt,draw=blue, line width=0.4mm, minimum width=4cm,minimum height=6.5cm,label={[opacity=0,onslide={<2>,opacity=1},fill=white,xshift=-1.0cm, yshift=-7cm]\textcolor{blue}{Symmetry controller}}]  at (-90pt, -140pt){};
...
\begin{itemize}
	...
	\item SP considers the manipulated symmetries valid all the time
\end{itemize}

\vfill
\centering
In hybrid approach, SP must be able to identify\\ \textbf{valid symmetries}
\end{frame}
%% ----------------------------------------------------------------------------
\begin{frame}
...
\begin{itemize}
	\item Combination of ESBP with other dynamic symmetry breaking approach
