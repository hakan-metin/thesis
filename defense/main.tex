...
\begin{center}
	\visible<2->{YES!  \texttt{SAT}  $\enspace \alpha = (A,1), (B,2), (C, 3)$\\ }
	\visible<3->{Many solutions $\alpha' = (A,2), (B,3), (C, 1)$}\\
		 \visible<4->{$\alpha'' = \cdots$}
\end{center}
...
\begin{tikzpicture}[overlay]
	\node[rounded, thick, minimum width=2cm, minimum height=0.5cm,opacity=0,onslide={<4->,opacity=1}] (c) at (208pt, 176pt) {};
	\node[,opacity=0,onslide={<4->,opacity=1}] (ct) at ($(c) + (+90pt, 10pt)$)  {Clause};
	\draw [->,thick,opacity=0,onslide={<4->,opacity=1}] (ct.west) to [bend right=30] (c);
	
	\node[rounded, thick, minimum width=3cm, minimum height=6.5cm,opacity=0,onslide={<5->,opacity=1}](f) at (210pt, 93pt) {};
	\node[,opacity=0,onslide={<5->,opacity=1},align=center] (ft) at ($(f) + (+90pt, 0pt)$) {Conjunctive\\ Normal\\ Form (CNF)};
	\draw [->,thick,opacity=0,onslide={<5->,opacity=1}] (ft.west) to [bend right=0] (f);
...
\begin{frame}
	...
	\node (isfin) [question] {All vars\\ assigned?};
...
\node[opacity=0,onslide={<4,8>,opacity=1}] (learn) [process]  at (280pt, 198pt) {Learn conflict clause\\ and backjump};%
...
\vfill
	...
	\vfill
	Forbids other assignments in each orbit\\
...
\begin{tikzpicture}
	...
	\node[box,minimum height=1.2cm,onslide={<3>, starburst,draw=red}] (sbp) at (0, -2) { SBPs};
...
\begin{column}[t]{.6\textwidth}
	...
	\end{itemize}
	\centering \visible<4->{\textbf{Inductive construction}}
\end{column}
\end{columns}
\end{frame}
...
\frametitle{Conclusion \& Perspectives}
	...
	\begin{itemize}
		...
		\item \textcolor{orange}{Combination of  CDCL[Sym] with SEL}
	\end{itemize}
\end{itemize}
\vfill
 Perspectives
