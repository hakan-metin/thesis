...
\hakan{REVOIR CORRECTION}
In this particular form, the given CNF formula contains only binary clauses.
In this case, the formula is transformed into a so called binary implication graph where each clause is transformed into an implication.
...
simultaneously to the true and false values. \Cref{fig:conflict} shows an example that leads to a conflict.
...
that are responsible of the contradiction and the \emph{conflict side} that contains the conflict. 
 A UIP is always in the reason side. \Cref{fig:implication-graph} depicts two cuts in the implication graph.
...
This is the key point of the CDCL algorithm and the big difference with the DPLL algorithm.
 In our example \Cref{fig:implication-graph}, the target decision level is 1.
...
\paragraph{Decision heuristics} \hakan{REVOIR}
